...
const Expenses = () => {
  ...
  const deleteExpenseHandler = () => {
    fetch(
      `https://expensetracker-52ef7-default-rtdb.asia-southeast1.firebasedatabase.app/expense${email}/${expenseToDelete.id}.json`,
      {
        method: "DELETE",
      }
    ).then((res) => {
  ...
  const handleEditExpense = (event) => {
    ...
    fetch(
      `https://expensetracker-52ef7-default-rtdb.asia-southeast1.firebasedatabase.app/expense${email}/${updatedExpense.id}.json`,
      {
        method: "PUT",
  ...
  useEffect(() => {
    const csv = expenses.reduce((csvString, expense) => {
      return `${csvString}  ₹${expense.amount}${expense.description},${expense.category}\n`;
